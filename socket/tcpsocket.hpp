...
class TcpSocket
{
    ...
    public:
        ...
        //服务端新建连接
        bool Accept(TcpSocket *sock, std::string *ip = NULL, uint16_t *port = NULL) //设置默认参数为空
        {
            //accept(监听套接字描述符，客户端地址信息，地址信息长度)
            //返回的操作句柄赋值传入的sock的成员sockfd中，通过sock与客户端通信
        ...
        {
            //recv(描述符，缓冲区，想要的数据长度，选项参数--0表述默认阻塞);
            char tmp[4096] = {0};
            ...
            buf->assign(tmp, ret);//从tmp中拷贝ret大小的数据到bu中
            return true;
        }
